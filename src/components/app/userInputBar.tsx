...
const UserInputBar: React.FC<UserInputBarProps> = ({ onSubmit, isLoading, showNote = true }) => {
  ...
  return (
    <div className='flex flex-col justify-center text-center items-center w-full'>
      ...
      {
        showNote &&
        <p className='text-xs sm:text-sm text-muted-foreground pt-2 max-w-[90%] mx-auto'>JAC BOT can make mistakes. Please double-check responses and refer to website <a href="https://jacchd.admissions.nic.in/" target="_blank" rel="noopener noreferrer" className='text-amber-500 hover:text-amber-600 underline'>https://jacchd.admissions.nic.in/</a></p>
      }
    </div>
  );
};
